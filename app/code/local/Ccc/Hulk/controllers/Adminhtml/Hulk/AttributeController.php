...
class Ccc_Hulk_Adminhtml_Hulk_AttributeController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('hulk/session');
    }
    ...
    public function preDispatch()
    {
        ...
        parent::preDispatch();
        $this->_entityTypeId = Mage::getModel('eav/entity')->setType(Ccc_Hulk_Model_Resource_Hulk::ENTITY)->getTypeId();
    }
    ...
    protected function _initAction()
    {
        ...
        if ($this->getRequest()->getParam('popup')) {
            ...
            $this->loadLayout()
                ...
                ->_addBreadcrumb(Mage::helper('hulk')->__('Hulk'), Mage::helper('hulk')->__('Hulk'))
                ->_addBreadcrumb(
                    Mage::helper('hulk')->__('Manage Attributes'),
                    Mage::helper('hulk')->__('Manage Attributes')
                );
        }
        return $this;
    ...
    public function indexAction()
    {
        ...
        $this->_addContent($this->getLayout()->createBlock('hulk/adminhtml_hulk_attribute'));
    ...
    public function editAction()
    {
        ...
        $model = Mage::getModel('hulk/resource_eav_attribute')
        ...
        $data = Mage::getSingleton('hulk/session')->getAttributeData(true);
        if (!empty($data)) {
            $model->addData($data);
        }
        ...
        $item = $id ? Mage::helper('hulk')->__('Edit Hulk Attribute')
            : Mage::helper('hulk')->__('New Hulk Attribute');
    ...
    public function validateAction()
    {
        ...
        $attribute = Mage::getModel('hulk/resource_eav_attribute')
            ->loadByCode($this->_entityTypeId, $attributeCode);

        if ($attribute->getId() && !$attributeId) {
            Mage::getSingleton('hulk/session')->addError(
                Mage::helper('hulk')->__('Attribute with the same code already exists')
            );
            $this->_initLayoutMessages('hulk/session');
    ...
    protected function _filterPostData($data)
    {
        if ($data) {
            ...
            $helperCatalog = Mage::helper('hulk');
    ...
    public function saveAction()
    {
        ...
        if ($data) {
            ...
            $session = Mage::getSingleton('hulk/session');

            $redirectBack   = $this->getRequest()->getParam('back', false);

            $model = Mage::getModel('hulk/resource_eav_attribute');

            $helper = Mage::helper('hulk/hulk');
            ...
            if (isset($data['attribute_code'])) {
                ...
                if (!$validatorAttrCode->isValid($data['attribute_code'])) {
                    $session->addError(
                        Mage::helper('hulk')->__('Attribute code is invalid. Please use only letters (a-z), numbers (0-9) or underscore(_) in this field, first character should be a letter. Do not use "event" for an attribute code.')
                    );
            ...
            try {
                $model->save();
                $session->addSuccess(
                    Mage::helper('hulk')->__('The Hulk attribute has been saved.')
                );
